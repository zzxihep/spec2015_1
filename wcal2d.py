...
def combine_lamp(lstfile):
    ...
    iraf.imcombine(input = '%ftbo%ftbo%@'+lstfile
        , output = lstfile.replace('.lst',''), combine = 'sum', reject = 'none')
...
def wal(lstfile,lampname):
   ...
   #     autowrite = 'no')
    ...
    iraf.reidentify(reference = lampname
        , images = lampname, interactive = 'no', section = 'column'
    ...
    iraf.fitcoords(images = lampname
        , fitname = lampname, interactive = True, combine = False, database = 'database'
    ...
    iraf.transform(input = '%ftbo%ftbo%@' + lstfile
        ...
        , fitnames = lampname+lampname, database = 'database', interptype = 'spline3'
        , flux = True)
...
def classify_lamp():
    ...
    for fitname in namelst:
        ...
        sidminute = int(sidminute[0])*60.0+int(sidminute[1])+float(sidminute[2])/60.0
        timelst.append(sidminute) #local Sidereal time
        altlst.append(fit[0].header['ALTPOS'])#Altitude Position of Telesocpe
    ...
    for fitname in corhalogen:
        fit = pyfits.open(fitname)
        sidminute = fit[0].header['LST'].split(':')
        sidminute = int(sidminute[0])*60.0+int(sidminute[1])+float(sidminute[2])/60.0+30
    ...
    for  i in range(len(subclass)):
        if len(ugly[i]) > 0:
            ...
            fil = open(subclass[i][0][0].replace('.fits','')+'lamp.lst','w')
            for j in range(len(subclass[i])):
                fil.write(subclass[i][j][0]+'\n')
            fil.close()
            fil = open(subclass[i][0][0].replace('.fits','')+'cor_halogen.lst','w')
