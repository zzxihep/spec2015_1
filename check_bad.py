...
def check_bias():
    ...
    for i in fitlst:
        ...
        if std > 50:
            ...
            flag = False
        if linemax > mean + 200:
            print ('linemax of %s = %.2f out of range %.2f + 200. '
                   'may be it is a bad line') % (i, linemax, mean)
            flag = False
        if linemin < mean - 200:
            print ('linemin of %s = %.2f out of range %.2f - 200. '
                   'may be it is a bad line') % (i, linemin, mean)
            flag = False
        if colmax > mean + 200:
            print ('colmax of %s = %.2f out of range %.2f + 200. '
                   'may be it is a bad col') % (i, colmax, mean)
            flag = False
        if colmin < mean - 200:
            print ('colmin of %s = %.2f out of range %.2f - 200. '
                   'may be it is a bad col') % (i, colmin, mean)
            flag = False
...
def check_halogen(abspathfile):
    ...
    for i in fitlst:
        ...
        print('%s(maxindex, val) %d %.2f' %
              (i, maxindex+1500, center[maxindex]))
        if center[maxindex] > 280000. or center[maxindex] < 150000.:
            ...
            if len(np.where(maxpart > 250000.)) > len(maxpart)/2:
                print('the max val in %s > 280000 %.3f' %
                      (i, center[maxindex]))
                flag = False
            if len(np.where(maxpart < 100000.)) > len(maxpart)/2:
                print('the max val in %s < 150000 %.3f' %
                      (i, center[maxindex]))
                flag = False
        if flag is True:
            goodlst.append(i)
...
def main():
    ...
    dirname = [i for i in dirname if os.path.isdir(i) and
               'bias' not in i and 'other' not in i]
    for i in dirname:
        check_other(i)
